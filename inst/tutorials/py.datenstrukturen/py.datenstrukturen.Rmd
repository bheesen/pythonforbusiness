...
Wird nur ein einzelner Wert in einer Variablen gespeichert, so bezeichnet man diese als einfache Variable bzw. elementare Datentypen. Werden mehrere Werte in einer Variablen gespeichert, so bezeichnet man diese als Vektor bzw. Datenstruktur. Die wichtigsten nicht elementaren Daten-typen, also Datenstrukturen, in Python sind:
**Liste (list)** und **Tupel (tuple)** sind Vektoren aus einer geordneten Liste von Elementen, welche unterschiedlichen Typs sein können. Die einzelnen Elemente lassen sich über einen Index ansprechen.**Set (set)** sind Mengen von nicht geordneten Elementen, bei denen doppelte Elemente nicht existieren.	**Dictionary (dict)** sind Lexika, d.h. es gibt einen Schlüsselbegriff (key), dem etwas zugeordnet wird, ähnlich einem Wörterbuch, bei dem einem Wort dessen Übersetzung zugeordnet wird. **Array (Ndarray)** sind N-dimensionale Arrays, welche besonders für die Verarbeitung von numerischen Arrays sehr verbreitet sind. Um diese Datenstruktur nutzen zu können muss zuvor das Paket numpy installiert sein. **Dataframe (dataframe)** und **Serie (series)** sind 2-dimensionale Data-frames, welche besonders für die Verarbeitung von Tabellen sehr verbreitet sind. Um diese Datenstruktur nutzen zu können muss zuvor das Paket pandas installiert sein.
...
Da Tupel unveränderlich sind, eignen sie sich zur Definition von Konstan-ten. Wenn eine Variable nur eine kleine Anzahl von benennbaren Zustän-den hat, z.B. Wochentage, Monate, Noten können mit dem Paket enums (enumeration bzw. Aufzählung) Konstantenklassen festgelegt werden (siehe nachfolgende Kodierung).
...
Listen können selbst wieder Listen als Elemente enthalten und somit ineinander geschachtelt sein, wie z.B. eine Matrix. Nachfolgend wird gezeigt, wie derartige Listen entschachtelt werden können.
...
Tupel sind eine Möglichkeit eine Konstante Liste zu definieren. Alternativ kann auch eine eigene Klasse als Konstante erstellt werden und einem Text eine Zahl zuordnen.
...
for tag in wochentag:                     # Schleife über Konstante
    if tag.name == "Montag":
        wert = 1
    elif tag.name == "Dienstag":
        wert = 2
    elif tag.name == "Mittwoch":
        wert = 3
    elif tag.name == "Donnerstag":
        wert = 4
    elif tag.name == "Freitag":
        wert = 5
    elif tag.name == "Samstag":
        wert = 6
    else:
        wert = 7
    print(tag,": Name:",tag.name,"=",wert)
...
```{python 1_4, exercise=TRUE, exercise.lines=7}
```
```{python 1_4-hint-1}
...
aktien = ["Apple", "Microsoft", "Tesla", "Amazon", "Google"]
preise = [175, 320, 600, 135, 2800]
```

Aufgabenstellung: Ein neuer Wert soll zum Portfolio hinzugefügt werden: Meta mit einem Preis von 450.
Der Investor verkauft seine Tesla-Aktien. Entferne Tesla und ihren Preis aus den Listen.
Ein Fehler wurde entdeckt: Google sollte eigentlich Alphabet heißen. Ändere den Namen in der Liste.
Berechne den durchschnittlichen Aktienpreis im Portfolio.S ortiere das Portfolio nach den Aktienpreisen (niedrigster Preis zuerst). Finde die teuerste Aktie im Portfolio. Füge eine neue Aktie Nvidia hinzu, aber an zweiter Stelle in der Liste.

Die Anzeige sollte hinterher wie folgt aussehen: ![Liste](images/liste.jpg)
...
```
```{python 1_5-hint-2}
index_tesla = aktien.index("Tesla")
del aktien[index_tesla]
del preise[index_tesla]
```
```{python 1_5-hint-3}
...
Die Anzeige sollte hinterher wie folgt aussehen: ![Sets](images/set.jpg)

```{python 2_4, exercise=TRUE, exercise.lines=3, exercise.setup="2_4-setup"}
```
```{python 2_4-hint-1}
alle_produkte = abteilung_1 | abteilung_2 | abteilung_3        # Vereinigung
print("Alle einzigartigen Produkte:", alle_produkte)
```
```{python 2_4-hint-2}
gemeinsame_produkte = abteilung_1 & abteilung_2 & abteilung_3  # Schnittmenge
print("Produkte in jeder Abteilung:", gemeinsame_produkte)     # Kein Produkt ist in allen drei Abteilungen
```
```{python 2_4-hint-3}
nur_in_abteilung_1 = abteilung_1 - (abteilung_2 | abteilung_3)  # Differenz
print("Produkte nur in Abteilung 1:", nur_in_abteilung_1)
```
```{python 2_4-hint-4}
in_mindestens_zwei = (abteilung_1 & abteilung_2) | (abteilung_1 & abteilung_3) | (abteilung_2 & abteilung_3)
print("Produkte in mindestens zwei Abteilungen:", in_mindestens_zwei)
```
```{python 2_4-hint-5}
print("Alle einzigartigen Produkte:", alle_produkte)
print("Produkte in jeder Abteilung:", gemeinsame_produkte)     # Kein Produkt ist in allen drei Abteilungen
...
bewertung_items=bewertung.items()
type(bewertung_items)
bewertung_items
#  Sortierung nach Key (Index 0)
bewertung_items_sortiert={key:value for (key,value) in sorted(bewertung.items(), key=lambda x:x[0])}
bewertung_items_sortiert
#  Sortierung nach Attribut (Index 1)
bewertung_items_sortiert={key:value for (key,value) in sorted(bewertung.items(), key=lambda x:x[1])}
bewertung_items_sortiert
...
Aufgabenstellung: Füge Meta mit einem Gewinn von 110 Millionen EUR als neues Unternehmen hinzu. Der Investor verkauft seine Anteile an Tesla. Entferne Tesla aus dem Dictionary.Der Gewinn von Amazon ist um 10% gestiegen. Aktualisiere den Wert. Bestimme den durchschnittlichen Gewinn aller Unternehmen.Ermittle das Unternehmen mit dem höchsten und niedrigsten Gewinn. Gib das Portfolio sortiert nach Gewinnen (aufsteigend) aus. Erstelle eine Liste aller Unternehmensnamen aus dem Dictionary.

Die Anzeige sollte hinterher wie folgt aussehen: ![Dictionary](images/dictionary.jpg)

```{python 3_3, exercise=TRUE, exercise.lines=3, exercise.setup="3_3-setup"}
```
```{python 3_3-hint-1}
...
```
```{python 3_3-hint-4}
unternehmen_sortiert = dict(sorted(unternehmen.items(), key=lambda x: x[1]))
...
print("Liste aller Unternehmen:", unternehmensnamen)
```
...
Die Anzeige sollte hinterher wie folgt aussehen: ![Array](images/array.jpg)
...
Um einen ersten Überblick über die Daten eines Dataframes oder Series-Objektes zu erhalten ist die Methode describe() geeignet.  Die Methode zeigt für alle numerischen Spalten die Anzahl, Mittelwert, Standardabweichung, Minimum, Maximum und die Quartile an.
...
Sie sind ein Finanzanalyst und möchten die Gewinne verschiedener Technologieunternehmen im Jahr 2024 analysieren. Nutzen Sie Pandas, um die Daten in dem DataFrame df zu verarbeiten.
...
df.to_csv("unternehmensfinanzen.csv", index=False)
